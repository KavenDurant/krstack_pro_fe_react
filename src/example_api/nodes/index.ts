...
const getPhysicalInfo = async (uid: string) =>
  get({ url: `nodes/detail/${uid}` });

// 云主机列表 http://192.168.1.213:8001/nodes/vms/${uid}
const getPhysicalInfoVmList = async (uid: string) =>
  get({ url: `nodes/vms/${uid}` });
// networkList  http://192.168.1.60:8686/api/nodes/cluster/180/node/host237/network
const getNetWorkList = async (uid: string) =>
  get({ url: `nodes/network/${uid}` });
...
const getSettingList = async (uid: string) =>
  get({ url: `nodes/network/settings/${uid}` });
